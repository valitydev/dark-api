...
import dev.vality.questionary.BusinessInfo;
import dev.vality.swag.questionary.model.*;
import dev.vality.swag.questionary.model.BusinessInfo.BusinessInfoTypeEnum;
import org.springframework.stereotype.Component;

@Component
public class BusinessInfoConverter implements
        ThriftConverter<BusinessInfo, dev.vality.swag.questionary.model.BusinessInfo>,
        SwagConverter<dev.vality.swag.questionary.model.BusinessInfo, BusinessInfo> {

    @Override
    public dev.vality.swag.questionary.model.BusinessInfo toSwag(BusinessInfo value, SwagConverterContext ctx) {
    ...
    @Override
    public BusinessInfo toThrift(dev.vality.swag.questionary.model.BusinessInfo value, ThriftConverterContext ctx) {
        BusinessInfo businessInfo = new BusinessInfo();
        switch (value.getBusinessInfoType()) {
            case ANOTHERBUSINESS:
                AnotherBusiness swagAnotherBusiness = (AnotherBusiness) value;
                businessInfo.setAnotherBusiness(new dev.vality.questionary.AnotherBusiness()
                        .setDescription(swagAnotherBusiness.getDescription()));
                return businessInfo;
            case BUILDINGBUSINESS:
                businessInfo.setBuildingBusiness(new dev.vality.questionary.BuildingBusiness());
                return businessInfo;
            case MEDIATIONINPROPERTYBUSINESS:
                businessInfo.setMediationInPropertyBusiness(new dev.vality.questionary.MediationInPropertyBusiness());
                return businessInfo;
            case PRODUCTIONBUSINESS:
                businessInfo.setProductionBusiness(new dev.vality.questionary.ProductionBusiness());
                return businessInfo;
            case RETAILTRADEBUSINESS:
                businessInfo.setRetailTradeBusiness(new dev.vality.questionary.RetailTradeBusiness());
                return businessInfo;
            case SECURITIESTRADINGBUSINESS:
                businessInfo.setSecuritiesTradingBusiness(new dev.vality.questionary.SecuritiesTradingBusiness());
                return businessInfo;
            case TRANSPORTBUSINESS:
                businessInfo.setTransportBusiness(new dev.vality.questionary.TransportBusiness());
                return businessInfo;
            case WHOLESALETRADEBUSINESS:
                businessInfo.setWholesaleTradeBusiness(new dev.vality.questionary.WholesaleTradeBusiness());
                return businessInfo;
            default:
