...
public class PartyContentSwagConverter
        implements SwagConverter<PartyContent, dev.vality.questionary_proxy_aggr.dadata_party.PartyContent> {

    @Override
    public PartyContent toSwag(dev.vality.questionary_proxy_aggr.dadata_party.PartyContent value,
        ...
        if (value.getType() == dev.vality.questionary_proxy_aggr.base_dadata.OrgType.LEGAL) {
            partyContent.setOrgType(OrgType.LEGAL);
        } else if (value.getType() == dev.vality.questionary_proxy_aggr.base_dadata.OrgType.INDIVIDUAL) {
    ...
    private CitizenshipIP convertCitizenshipIP(
            dev.vality.questionary_proxy_aggr.base_dadata.CitizenshipIP citizenshipIP) {
    ...
    private PartyAuthorities convertPartyAuthorities(
            dev.vality.questionary_proxy_aggr.dadata_party.PartyAuthorities partyAuthorities) {
    ...
    private PartyCapital convertPartyCapital(
            dev.vality.questionary_proxy_aggr.dadata_party.PartyCapital partyCapital) {
        return new PartyCapital()
                .type(partyCapital.getType())
                .value(partyCapital.getValue());
    }

    private OrgName convertOrgName(dev.vality.questionary_proxy_aggr.base_dadata.OrgName orgName) {
        return new OrgName()
    ...
    private Authorities convertAuthorities(dev.vality.questionary_proxy_aggr.base_dadata.Authorities authorities) {
    ...
    private PartyOkved convertPartyOkved(dev.vality.questionary_proxy_aggr.dadata_party.PartyOkved partyOkved) {
    ...
    private Management convertManagement(dev.vality.questionary_proxy_aggr.base_dadata.Management management) {
        return new Management()
                .post(management.getPost())
                .name(management.getName());
    }

    private Manager convertManager(dev.vality.questionary_proxy_aggr.base_dadata.Manager manager) {
        ...
        if (manager.getType() == dev.vality.questionary_proxy_aggr.base_dadata.ManagerType.LEGAL) {
            swagManager.setType(ManagerType.LEGAL);
        } else if (manager.getType() == dev.vality.questionary_proxy_aggr.base_dadata.ManagerType.EMPLOYEE) {
            swagManager.setType(ManagerType.EMPLOYEE);
        } else if (manager.getType() == dev.vality.questionary_proxy_aggr.base_dadata.ManagerType.FOREIGNER) {
