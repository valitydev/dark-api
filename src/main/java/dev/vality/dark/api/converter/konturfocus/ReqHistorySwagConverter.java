...
public class ReqHistorySwagConverter
        implements SwagConverter<ReqHistory, dev.vality.questionary_proxy_aggr.kontur_focus_req.ReqHistory> {

    @Override
    public ReqHistory toSwag(dev.vality.questionary_proxy_aggr.kontur_focus_req.ReqHistory value,
        ...
        if (value.isSetKpps()) {
            List<ReqKppHistory> swagReqKppHistoryList = value.getKpps().stream()
                    .map(reqKppHistory -> ctx.convert(reqKppHistory, ReqKppHistory.class))
                    .collect(Collectors.toList());
        ...
        if (value.isSetLegalNames()) {
            List<LegalName> swagLegalNameList = value.getLegalNames().stream()
                    .map(legalName -> ctx.convert(legalName, LegalName.class))
                    .collect(Collectors.toList());
        ...
        if (value.isSetLegalAddresses()) {
            List<LegalAddress> swagLegalAddressList = value.getLegalAddresses().stream()
                    .map(legalAddress -> ctx.convert(legalAddress, LegalAddress.class))
                    .collect(Collectors.toList());
